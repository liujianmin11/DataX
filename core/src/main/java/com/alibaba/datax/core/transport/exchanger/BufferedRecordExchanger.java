...
public class BufferedRecordExchanger implements RecordSender, RecordReceiver {
    ...
    public void sendToWriter(Record record) {
        ...
        if (record.getMemorySize() > this.byteCapacity) {
            if ((record.getMemorySize() < this.byteCapacity * 10)) {
                this.pluginCollector.collectDirtyRecord(record, new Exception(String.format("单条记录超过大小限制，当前限制为:%s， record:%s",
                        this.byteCapacity, record)));
            } else {
                this.pluginCollector.collectDirtyRecord(record, new Exception(String.format("单条记录超过大小限制，当前限制为:%s， size:%s",
                        this.byteCapacity, String.valueOf(record.getMemorySize()))));
            }
            return;
        }
